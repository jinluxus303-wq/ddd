  ...
  try {
    ...
    if (cached) {
        ...
        return NextResponse.json(
          {
            success: false,
            error:
              "STEP1_FAILED: All ShowBox workers failed or returned no share link",
          },
          { status: 502 },
        );
      }
      ...
      if (!shareRes.ok) {
        return NextResponse.json(
          {
            success: false,
            error: `STEP2_FAILED: FebBox share worker returned ${shareRes.status}`,
          },
          { status: 502 },
        );
      }
      ...
      if (!files.length) {
        return NextResponse.json(
          {
            success: false,
            error: "STEP2_EMPTY: No files in FebBox share",
            share: shareData,
          },
          { status: 404 },
        );
    ...
    if (!playerRes.ok) {
      return NextResponse.json(
        {
          success: false,
          error: `STEP3_FAILED: FebBox player worker returned ${playerRes.status}`,
          from_db: fromDb,
          fid,
        },
        { status: 502 },
    ...
    if (!playerData.success) {
      return NextResponse.json(
        {
          success: false,
          error: "STEP3_PARSE_FAILED: FebBox player returned no streams",
          from_db: fromDb,
          fid,
          detail: playerData,
        },
    ...
    if (!finalM3u8Url) {
      return NextResponse.json(
        {
          success: false,
          error: "STEP3_NO_URL: No stream URL found in player response",
          from_db: fromDb,
          fid,
          streams,
        },
